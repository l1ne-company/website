...
export default function Logo3D() {
  ...
  const sourceCode = `// L1ne Company Logo - GLSL Raymarching Shader
// Fragment Shader
...
float map(vec3 p) {
  float heights[8];
  heights[0] = 1.2; heights[1] = 1.5; heights[2] = 1.8; heights[3] = 2.1;
  heights[4] = 2.4; heights[5] = 2.1; heights[6] = 1.8; heights[7] = 1.5;

  float d = 1000.0;
  for (int i = 0; i < 8; i++) {
    vec3 q = p;
    q.x -= (float(i) - 3.5) * 0.5;
    q.y -= sin(u_time * 0.5 + float(i) * 0.5) * 0.1;
    d = min(d, sdBox(q, vec3(0.3, heights[i], 0.3)));
  }
  return d;
}
